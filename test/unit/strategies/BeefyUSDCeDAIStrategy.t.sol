...
import { _1_DAI, _1_USDCE } from "test/helpers/Tokens.sol";
...
contract BeefyUSDCeDAIStrategyTest is BaseTest, ConvexdETHFrxETHStrategyEvents {
    ...
    function testBeefyUSDCeDai___SimulateHarvest() public {
        vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
        vault.deposit(100 * _1_USDCE, users.alice);
        vm.startPrank(users.keeper);
    ...
    // function testBeefyUSDCeDai__PreviewLiquidateExact_FUZZY(uint256 amount) public {
    //     vm.assume(amount > _1_USDCE && amount < 1_000_000 * _1_USDCE);
    //     deal(USDCE_POLYGON, users.alice, amount);
    //     vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
    //     vault.deposit(amount, users.alice);
    ...
    // function testBeefyUSDCeDai__maxLiquidateExact_FUZZY(uint256 amount) public {
    //     vm.assume(amount > _1_USDCE && amount < 1_000_000 * _1_USDCE);
    //     deal(USDCE_POLYGON, users.alice, amount);
    //     vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
    //     vault.deposit(amount, users.alice);
    ...
    // function testBeefyUSDCeDai__MaxLiquidate_FUZZY(uint256 amount) public {
    //     vm.assume(amount > _1_USDCE && amount < 1_000_000 * _1_USDCE);
    //     deal(USDCE_POLYGON, users.alice, amount);
    //     vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
    //     vault.deposit(amount, users.alice);
