...
import { BeefyCrvUSDUSDTStrategyWrapper } from "../../mock/BeefyCrvUSDUSDTStrategyWrapper.sol";
import { _1_USDCE } from "test/helpers/Tokens.sol";
import { SafeTransferLib } from "solady/utils/SafeTransferLib.sol";

contract BeefyCrvUSDUSDTStrategyTest is BaseTest, ConvexdETHFrxETHStrategyEvents {
    using SafeTransferLib for address;

    address public TREASURY;
    IStrategyWrapper public strategy;
    BeefyCrvUSDUSDTStrategyWrapper public implementation;
    ...
    function setUp() public {
        ...
        implementation = new BeefyCrvUSDUSDTStrategyWrapper();
    ...
    function testBeefyCrvUSDUSDCe__Initialization() public {
        ...
        BeefyCrvUSDUSDTStrategyWrapper _implementation = new BeefyCrvUSDUSDTStrategyWrapper();
        ...
        assertEq(IERC20(USDCE_POLYGON).allowance(address(_strategy), CURVE_CRVUSD_USDT_POOL_POLYGON), type(uint256).max);
    ...
    function testBeefyCrvUSDUSDCe__IsActive() public {
        ...
        strategy.divest(IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)));
    ...
    function testBeefyCrvUSDUSDCe__Invest() public {
        ...
        // assertEq(returned, 0);
        // assertEq(IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)), 0);
        ...
        assertApproxEq(
            expectedShares, IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)), expectedShares / 100
        );
    }
    ...
    function testBeefyCrvUSDUSDCe__Divest() public {
        ...
        uint256 amountDivested = strategy.divest(IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)));
    ...
    function testBeefyCrvUSDUSDCe__LiquidatePosition() public {
        ...
        assertApproxEq(liquidatedAmount, 50 * _1_USDCE, 50 * _1_USDCE / 100);
        assertLt(loss, _1_USDCE / 5);
    }
    ...
    function testBeefyCrvUSDUSDCe__LiquidateAllPositions() public {
        ...
        assertApproxEq(IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)), shares, shares / 100);
        ...
        assertApproxEq(IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)), shares, 1.5 ether);
    ...
    function testBeefyCrvUSDUSDCe__Harvest() public {
        ...
        strategy.harvest(0, 0, address(0), block.timestamp);
        assertEq(IERC20(USDCE_POLYGON).balanceOf(address(vault)), 109_964_840);
        assertEq(IERC20(BEEFY_CRVUSD_USDT_POLYGON).balanceOf(address(strategy)), 0);
        vm.revertTo(snapshotId);
