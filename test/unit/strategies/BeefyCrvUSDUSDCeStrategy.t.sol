...
contract BeefyCrvUSDUSDCeStrategyTest is BaseTest, ConvexdETHFrxETHStrategyEvents {
    ...
    function testBeefyCrvUSDUSDCe__PreviewLiquidate__FUZZY(uint256 amount) public {
    ...
    function testBeefyCrvUSDUSDCe__PreviewLiquidateExact__FUZZY(uint256 amount) public {
        vm.assume(amount > _1_USDCE && amount < 1_000_000 * _1_USDCE);
        deal(USDCE_POLYGON, users.alice, amount);
        vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
        vault.deposit(amount, users.alice);
    ...
    function testBeefyCrvUSDUSDCe__maxLiquidateExact_FUZZY(uint256 amount) public {
        vm.assume(amount > _1_USDCE && amount < 1_000_000 * _1_USDCE);
        deal(USDCE_POLYGON, users.alice, amount);
        vault.addStrategy(address(strategy), 9000, type(uint72).max, 0, 0);
        vault.deposit(amount, users.alice);
    ...
    function testBeefyCrvUSDUSDCe__MaxLiquidate_FUZZY(uint256 amount) public {
        vm.assume(amount > _1_USDCE && amount < 1_000_000 * _1_USDCE);
        deal(USDCE_POLYGON, users.alice, amount);
        vault.addStrategy(address(strategy), 9000, type(uint72).max, 0, 0);
        vault.deposit(amount, users.alice);
    ...
    function testBeefyCrvUSDUSDCe___SimulateHarvest() public {
        vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
        vault.deposit(100 * _1_USDCE, users.alice);
        vm.startPrank(users.keeper);
