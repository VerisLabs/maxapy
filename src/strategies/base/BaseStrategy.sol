...
abstract contract BaseStrategy is Initializable, OwnableRoles {
    ...
    function _simulateHarvest() internal {
        ...
        revert(abi.encode(true,expectedBalance));
    }

    function simulateHarvest() public returns(uint256){
        try _simulateHarvest() {
        }catch(bytes memory e) {
           (bool success, uint256 expectedBalance) = abi.decode(e, (bool, uint256));
           if(!success) revert();
