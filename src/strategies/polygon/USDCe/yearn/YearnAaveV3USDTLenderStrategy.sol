...
/// @title YearnAAVEv3USDTLenderStrategy
/// @author Adapted from https://github.com/Grandthrax/yearn-steth-acc/blob/master/contracts/strategies.sol
/// @notice `YearnAAVEv3USDTLenderStrategy` supplies an underlying token into a generic Yearn Vault,
/// earning the Yearn Vault's yield
contract YearnAaveV3USDTLenderStrategy is BaseYearnV3Strategy {
        ...
        returns (uint256 depositedAmount)
    {
        ...
        amount = USDT_POLYGON.balanceOf(address(this)) - balanceBefore;
    ...
    function _divest(uint256 shares) internal override returns (uint256 withdrawn) {
        ...
        zapper.exchange_underlying(2, 1, withdrawn, 0, address(this));
    ...
    function _shareValue(uint256 shares) internal view override returns (uint256 _assets) {
        ...
        return zapper.get_dy_underlying(2, 1, _assets);
    }
    ...
    /// @notice Returns the price of token USDC<>USDT or USDT<>USDC withouth considering any slippage or fees
    ...
    function _spotPriceDy(uint256 i, uint256 amount) internal view returns (uint256 _spotDy) {
        if (i == 2) {
            uint256 spotPrice = zapper.get_dy_underlying(2, 1, 1e6);
            return spotPrice * amount / 1e6;
        }

        if (i == 1) {
            uint256 spotPrice = zapper.get_dy_underlying(1, 2, 1e6);
            return spotPrice * amount / 1e6;
        }
    }
}
